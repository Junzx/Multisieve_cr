...
def get_result(vars = 'train'):
    ...
    for file_idx, file_ in enumerate(test_files):
        ...
        prf = api_cr_one_file.main(file_)
    ...
    with open('all_result.' + api_cr_one_file.sieve_order[-1].__name__, 'wb') as hdl:
        cPickle.dump(all_res, hdl)

def test():
    with open('all_result.' + api_cr_one_file.sieve_order[-1].__name__, 'rb') as hdl:
...
if __name__ == '__main__':
    import time
    start = time.clock()
    print api_cr_one_file.sieve_order[-1].__name__
    # get_result('train')
