...
def extract_mention_for_mention_detection(lst_tokens, lst_key_words):
    """
    根据一个token list，根据最后一列信息，构建mention对象
    :param lst_token: 一列token 对象
    :return:[<__main__.NP object at 0x02B497F0>, <__main__.NP object at 0x02B49810>, ...] 返回的list中是np对象
    ...
    lst_key_word = [item['word'] for item in lst_key_words]
        ...
        for int_index_each_char, str_each_char in enumerate(str_iter_content):  # str_each_char是每个信息的每个字符
            if str_each_char == '(':
                ...
                if obj_mention.chinese_word in lst_key_word:  # 说明这个是关键字
                    obj_mention.weight = \
                        [item['weight'] for item in lst_key_words if item['word'] == obj_mention.chinese_word][0]
