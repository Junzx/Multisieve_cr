...
def Article2DocumentObject(article):
    obj_document = Document()
    obj_document.first_line = 'Init Data has no first line!'
    lst_every_sentences = [str_sent + '。' for str_sent in article.strip('。').split('。')]
    ...
    obj_document.api_set_mention_detection_attribute()
    return obj_document
...
if __name__ == '__main__':
    ...
    article = '特朗普访问中国，这是美国总统第n次来华访问。'
    data = Article2DocumentObject(article)
